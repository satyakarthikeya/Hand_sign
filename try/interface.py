...
CNN_MODEL_PATH = os.path.join('models', 'cnn_model.pt')  # Add CNN model path
YOLO_MODEL_PATH = 'models\best.pt'  # Path to the YOLO model for hand detection
...
class HandSignApp:
    ...
    def load_models(self):
        """Load VGG, ResNet and CNN models"""
        self.models = {}
    ...
    def create_ui(self):
        ...
        self.cnn_radio = ttk.Radiobutton(self.model_selection_frame, text="CNN Model", variable=self.model_var,
                                       value="CNN", command=self.set_model)
        self.cnn_radio.pack(side=tk.LEFT, padx=10, pady=5)
                ...
                if self.yolo_model is not None and self.yolo_enabled_var.get():
                    try:
                        ...
                        try:
                            ...
                            cv2.putText(frame, f"Hands detected: {detection_count}", (10, 90), 
                                ...
                                for box in boxes:
                                    try:
                                        ...
                                        if conf > conf_threshold:  
                                            ...
                                            cv2.rectangle(frame, label_bg_pt1, label_bg_pt2, box_color, -1)
    ...
    def preprocess_canvas(self):
        ...
        # Determine input size based on active model
