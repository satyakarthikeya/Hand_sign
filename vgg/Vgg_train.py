...
BATCH_SIZE = 32
EPOCHS = 70 # Training will run for all 150 epochs without early stopping
...
# Modified training function to remove early stopping
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    """
    Train the model for the full number of epochs (removed early stopping)
    """
    train_losses = []
    val_losses = []
    train_accs = []
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_acc > best_val_acc:
            ...
            best_epoch = epoch
            torch.save(model.state_dict(), 'hand_sign_vgg_model.pth')
    ...
    model.load_state_dict(torch.load('hand_sign_vgg_model.pth'))
...
def train_vgg_model():
    """
    Main function to train the VGG model
    """
    ...
    transform = transforms.Compose([
        transforms.RandomRotation(20),  # Increased rotation angle
        transforms.RandomAffine(0, translate=(0.15, 0.15)),  # More translation
        transforms.RandomAffine(0, scale=(0.85, 1.15)),  # More scaling
        transforms.RandomAffine(0, shear=10),  # Added shear augmentation
    ...
    # Split dataset with stratification to ensure balanced classes
    # For simplicity we'll use random_split as before, but in production
    # consider using StratifiedShuffleSplit from sklearn
    ...
    model = VGGModel(NUM_CLASSES).to(DEVICE)
    print(model)
    ...
    print(f"Will train for full {EPOCHS} epochs (no early stopping)")
    history = train_model(model, train_loader, val_loader, criterion, optimizer, EPOCHS)
    
    # Plot training history
    plot_training_history(history)
    
    print("Model training completed. Model saved as 'hand_sign_vgg_model.pth'")
    
    # Save model for mobile deployment 
    # Export to TorchScript
    example = torch.rand(1, 3, IMG_SIZE, IMG_SIZE).to(DEVICE)
    traced_script_module = torch.jit.trace(model, example)
    traced_script_module.save("hand_sign_model_mobile.pt")
    
    print("Mobile-optimized model saved as 'hand_sign_model_mobile.pt'")
